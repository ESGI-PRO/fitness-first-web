...
export default function TrainerPage({ user }) {
  ...
  const RenderTraineeList = () => {
    return users?.length > 0 ? users?.map((item, index) => {
      return <RenderItem item={item} key={index} index={index} />;
    }) : (
        <>
            <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Vous n'avez toujours pas d'éléves associés ! Veuillez attendre qu'un eleve vous soit assigné </h1>
        </>
    );
  };
    ...
    <>
      ...
      <Container>
        <div className="flex flex-1 mt-8  flex-col">
          {state.openAssignExercises && (
            <AssignExercisesPage
              setOpenAssignExercises={setOpenAssignExercises}
            />
          )}
          {state.trainerPage && <RenderTraineeList />}
          {state.userList && (
            <UserTrainingList setOpenUserList={setOpenUserList} />
          )}{" "}
        </div>
      </Container>
    </>
