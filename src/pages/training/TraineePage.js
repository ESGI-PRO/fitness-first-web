  ...
  const RenderItem = ({item, index}) => {
    ...
    return (
        <div className='mx-4 flex-col flex w-full'>
            ...
            {
            index !== trainers.length - 1 && <div className='my-8 w-full grayColorBackground'
                style={
                    {height: 0.5}
                }/>
        } </div>
...
export default function TraineePage() {
  ...
  const updateUser = async () => {
    ...
    return (
            <Container> {
                user?.trainerId ?  <TrainneeTrainingPage user={user} /> : <AssignTrainerPage updateUser={updateUser} user={user}/>
            } </Container>
    );
}
